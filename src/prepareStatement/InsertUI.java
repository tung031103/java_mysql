...
public class InsertUI extends JFrame{
    ...
    protected void xuuLyLuu() {
        try {
            ...
            preparedStatement.setInt(4, Integer.parseInt(txtSoNamKhauHao.getText()));
            preparedStatement.setInt(5, Integer.parseInt(txtGiaTri.getText()));

            int x =preparedStatement.executeUpdate();
            if(x > 0){
                JOptionPane.showMessageDialog(null, "Thêm tài sản thành công");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
